...
| **F3** | **High-κ gate dielectric** | 2007 (45nm): SiO₂ → HfO₂ | **gate tunneling leakage** (exp in `t_phys`) vs **`C_ox`** (linear in EOT) — one thickness, two currencies | **🔨 SLICE 1 BUILT (2026-07-15)** (`chip/high_k.py`) — EOT identity + per-material WKB tunneling; `device.py` untouched. Remaining: game knob, wiring, history mode, IL slice |
