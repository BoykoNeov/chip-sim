...
- [High-κ gate F3](high-k-gate-f3.md) — **project (2026-07-15 → COMPLETE 2026-07-17, 4 slices; card PULLED):** one thickness, two currencies — `C_ox` LINEAR in EOT vs `J_g` EXPONENTIAL in `t_phys`. **`device.py` untouched: `ε_SiO₂/EOT ≡ ε₀κ/t_phys` is an IDENTITY.** κ↔gap INVERSE ⇒ per-material `α∝√(m*φ_B)`. S4 = the **IL**: both currencies **additive over series layers** ⇒ the IL is the *better barrier* yet **a pure loss** (capacitance-only flips the sign); floor `EOT>t_IL` for ANY κ. (F4 shipped after it; then [[strained-silicon-f5]] was chosen over F8.)
- [Strained silicon F5](strained-silicon-f5.md) — **project (2026-08-10 → COMPLETE, 4 slices; card GRADUATED, F8 inherits the queue):** chosen over F8 (whose remaining gate is a *refactor*). `µ` = the one `I_Dsat` factor no process ever moved. Seam **predates** the slice (`mu_eff` defaulted since P4 ⇒ `device.py` untouched, 4th slice running). Carrier fork → **enhancement factors**, wired leg = nMOS tensile. Elasticity bound **≈0.5 cited on both carriers** ⇒ drive read is an UPPER BOUND, no elasticity knob. **S2 = the 1st NON-additive knob** (re-grades the wafer alone — F4 needed the pair); common-mode × ⇒ it's the LADDER moving, and the win **costs yield** (89/89 → 77/89). **S3 = B10** (slotted B8→B10→B6, process order): the **flattering-direction trap** at the *figure* layer, the structural zero under it, and the period that is a POINT not a curve. **S4 = the finale**: bracket-not-a-curve, the model blind to the decay by construction, and a house helper (`nodes_bought`) that carried the WRONG slice's denominator. Live: a published card's gate was already false.
- [CMP / planarity F8](cmp-planarity-f8.md) — **project (2026-08-19 → S3 2026-09-02; S4 open, card graduates with it):** the step upstream of F4's wire. S1: `s/(1−s)` forced overpolish (no constant; 0 at s=0 ⇒ fix = UNIFORMITY never polish-less), V barred by the kinematic identity ⇒ P carries the radius, sub-micron loss is EROSION (dishing exactly 0), `DISH_SCALE` a quarantined calibration. S2: the wire's first per-die spread (one transistor, many delays), damascene-only refusal (`DAMASCENE_METALS`), short graded by radius in closed form, grading by position. S3 = B11 (one transistor, many delays; s_crit; two levers + "polish less" is NOT a lever). Gate was FALSE on re-check (3rd time).
...
- [BEOL interconnect source + build](beol-interconnect-source.md) — **F4 COMPLETE (4 slices, 2026-07-17 → 2026-08-10; card PULLED):** cited `c_pul≈2 pF/cm` + its **invariance** ⇒ crossover driven by **R not C**. **SIGN TRAP:** Ru's bulk ρ is 4× *worse* than Cu. `L` = house lump ⇒ headline prefactor-free. **S2** = the binning inversion (exact `∂ln f/∂ln I=1−wire_share`; compression **SYMMETRIC** ⇒ grading loss, NOT yield). **S3** = B9: Cu bought **0.64 of a node**, **challenger-first** ordering, the bulk-ρ axis is out of metals. **S4** = the axis change, and the plan's "two steps" became **two IMPOSSIBILITY RESULTS**: the size effect alone **never** flips it (asymptote **1.179 > 1** — the FOM buys *parity*), the barrier alone only below **5.24 nm** (atop the **4.0 nm** all-barrier floor); together **4.23→1.90→0.92** at 12 nm. Crossing = a **BAND (12.9–17.1 nm) set by the cited `t_b`**. The **FOM ranks but does NOT locate** (deep-limit = 4× wrong). Live bug: Ru in `METALS` **reached the game knob** → `BULK_ERA_METALS`. Al **refused** on the narrow axis (its flattering `ρ₀λ` vs an EM disqualifier F4 can't carry). **F4 released F8's gate.**
- [Strained-silicon source + S1 build](strained-silicon-source.md) — **F5 S1 (2026-08-10):** Intel 90 nm uniaxial, **both legs one paper** (17% Ge→hole µ>50%/+25% drive; tensile cap→+20% µ/+10% drive). **Elasticity = ratio of FRACTIONAL gains** `(I−1)/(µ−1)` = **0.500 exactly**, NOT `1.10/1.20`=0.917 (the silent-failure trap). Model elasticity **1 exactly vs real `device.py`** — but ideal-contact path ONLY (`R_series>0` sub-linearizes on its own). `nmos_mobility` **REFUSES** the hole leg (wrong sign, not a guard rail); no stress field exists at all.
