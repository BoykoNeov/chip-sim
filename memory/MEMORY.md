...
- [Strained silicon F5](strained-silicon-f5.md) — **project (2026-08-10; S1+S2 BUILT, S3 open, S4 uncommitted):** chosen over F8 (whose remaining gate is a *refactor*). `µ` = the one `I_Dsat` factor no process ever moved. Seam **predates** the slice (`mu_eff` defaulted since P4 ⇒ `device.py` untouched, 4th slice running). Carrier fork → **enhancement factors**, wired leg = nMOS tensile. Elasticity bound **≈0.5 cited on both carriers** ⇒ drive read is an UPPER BOUND, no elasticity knob. **S2 = the 1st NON-additive knob** (moves `I_Dsat`, already graded ⇒ re-grades alone, no new scoring surface — F4 needed the pair); common-mode ×, so CV/rank invariant ⇒ it's the LADDER moving. The win **costs yield**: 89/89 → 77/89 on a ceiling written for the unstrained line. Live: a published card's gate was already false.
