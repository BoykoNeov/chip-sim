...
- [Strained silicon F5](strained-silicon-f5.md) — **project (PLANNED 2026-08-10, not built):** chosen over F8 (whose remaining gate is a *refactor*). `µ` = the one `I_Dsat` factor no process ever moved. Seam **predates** the slice (`mu_eff` defaulted since P4 ⇒ `device.py` untouched, 4th slice running). Carrier fork → **enhancement factors**, wired leg = nMOS tensile. Elasticity bound **≈0.5 cited on both carriers** ⇒ drive read is an UPPER BOUND, no elasticity knob. Live: a published card's gate was already false.
