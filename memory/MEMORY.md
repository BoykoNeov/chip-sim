...
- [BEOL interconnect source + build](beol-interconnect-source.md) — **F4 (S1–S3 BUILT 2026-07-17; S4 = LAST):** cited `c_pul≈2 pF/cm` + its **invariance** ⇒ crossover driven by **R not C**. **SIGN TRAP:** Ru's bulk ρ is 4× *worse* than Cu; `ρ₀λ` buys only **parity** — the unscalable **barrier** tips it. `L` = house lump ⇒ headline prefactor-free. **S2** = the binning inversion (exact `∂ln f/∂ln I=1−wire_share`; the market-promise edges; compression **SYMMETRIC** ⇒ grading loss, NOT yield). **S3** = the B9 demo: `W_x ∝ √ρ₀` ⇒ Cu bought **0.64 of a node** (never round a win up), `crossover_width_ratio` = **challenger-first** (a live sign trap), the **bulk-ρ axis is out of metals** (scope it to the axis or it contradicts S4), and `W_x ∝ √I_Dsat` ⇒ **the transistor's own progress creates the wire wall**.
