...
- [CMP / planarity F8](cmp-planarity-f8.md) — **project (2026-08-19 → S2 2026-09-02; S3/S4 open):** the step upstream of F4's wire. S1: `s/(1−s)` forced overpolish (no constant; 0 at s=0 ⇒ fix = UNIFORMITY never polish-less), V barred by the kinematic identity ⇒ P carries the radius, sub-micron loss is EROSION (dishing exactly 0), `DISH_SCALE` a quarantined calibration. S2: the wire's first per-die spread (one transistor, many delays), damascene-only refusal (`DAMASCENE_METALS`), short graded by radius in closed form, grading by position. Gate was FALSE on re-check (3rd time).
