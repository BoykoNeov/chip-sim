...
- [BEOL interconnect source + build](beol-interconnect-source.md) — **F4 (S1–S2 BUILT 2026-07-17; S3/S4 open):** cited `c_pul≈2 pF/cm` + its **invariance** (coax↔80nm line, 7 orders) ⇒ crossover driven by **R not C**; global-vs-local scaling is load-bearing. **SIGN TRAP:** Ru's bulk ρ is 4× *worse* than Cu and `ρ₀λ` (Cu 65 < Ru 77) only buys **parity** — the **barrier** (2–3nm, unscalable) tips it. `L` = house lump ⇒ headline prefactor-free. **S2** = the binning inversion: exact law `∂ln f/∂ln I=1−wire_share`; re-binning proves NOTHING unless the edges carry the *market promise* (`from_speed_bins`); compression is **SYMMETRIC** ⇒ grading loss, NOT yield. S1's "guard fires at the house operating point" **WITHDRAWN** (it was a test fixture's load).
